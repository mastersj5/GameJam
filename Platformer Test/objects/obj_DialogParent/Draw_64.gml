/// @description Parent Dialog object Draw GUI Event
...
if(showing_dialog == true){
	...
	draw_rectangle(border, border, display_get_gui_width() - border, height - border, false);//outer border around dialogue
	
	draw_set_color(c_black);
	draw_rectangle((border * 2), (border * 2), display_get_gui_width() - (border * 2), height - (border * 2), false);//inside border around dialogue
	
	if(current_dialog.sprite != -1){
		draw_sprite(current_dialog.sprite, current_dialog.subimage, border * 3, border * 3);	
	}
